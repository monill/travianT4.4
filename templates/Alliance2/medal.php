...
foreach ($varmedal as $medal) {
    ...
    if (isset($bonus[$medal['id']])) {
        $profiel = preg_replace("/\[#" . $medal['id'] . "]/is", '<img src="' . $gpack . '/t/' . $medal['img'] . '.gif" border="0" title="' . $titel . '<br />' . WEEK . ' ' . $medal['week'] . '">', $profiel, 1);
    } else {
        $profiel = preg_replace("/\[#" . $medal['id'] . "]/is", '<img src="' . $gpack . '/t/' . $medal['img'] . '.gif" border="0" title="' . JR_CATEGORY . ': ' . $titel . '<br />' . WEEK . ': ' . $medal['week'] . '<br />' . RANK . ': ' . $medal['plaats'] . '<br />' . $woord . ': ' . $medal['points'] . '<br />">', $profiel, 1);
    }
}
