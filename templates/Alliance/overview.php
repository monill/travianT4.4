        ...
        foreach ($memberlist as $member) {
            ...
            if ($aid == $session->alliance) {
                if ((time() - 600) < $member['timestamp']) { // 0 Min - 10 Min
                    echo "<img class='online online1' src=/assets/images/x.gif title='Online' alt='Online' />";
                } elseif ((time() - 86400) < $member['timestamp'] && (time() - 600) > $member['timestamp']) { // 10 Min - 1 Days
                    echo "<img class='online online2' src=/assets/images/x.gif title='Maximum 24 hours' alt='Maximum 24 hours' />";
                } elseif ((time() - 259200) < $member['timestamp'] && (time() - 86400) > $member['timestamp']) { // 1-3 Days
                    echo "<img class='online online3' src=/assets/images/x.gif title='Maximum 3 days' alt='Maximum 3 days' />";
                } elseif ((time() - 604800) < $member['timestamp'] && (time() - 259200) > $member['timestamp']) { // 3-7 Days
                    echo "<img class='online online4' src=/assets/images/x.gif title='Maximum 7 days' alt='Maximum 7 days' />";
                } else {
                    echo "<img class='online online5' src=/assets/images/x.gif title='Low labor' alt='Low labor' />";
                }
            }
