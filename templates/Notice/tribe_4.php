...
<table cellpadding="0" cellspacing="0">
    <thead>
        <tr>
            ...
            <td class="troopHeadline" colspan="<?php if ($dataarray[116]) {
                                                    echo '11';
                                                } else {
                                                    echo '10';
                                                } ?>">
                ...
                if ($targettribe == '4') {
                    ...
                    echo '</a> ' . REPORT_FROM_VIL . ' <a href="position_details.php?d=' . $dataarray[31] . '&amp;c=' . $generator->getMapCheck($dataarray[31]) . '">
    ...
    <tbody class="units">
        <tr>
            ...
            for ($i = $start; $i <= ($start + 9); $i++) {
                if ($i == ($start + 9) && !$dataarray[116]) {
                    $last = ' last';
                } else {
                    $last = '';
                }
                echo "<td class=\"uniticon" . $last . "\"><img src=\"/assets/images/x.gif\" class=\"unit u$i\" title=\"" . $technology->getUnitName($i) . "\" alt=\"" . $technology->getUnitName($i) . "\" /></td>";
            }
            if ($dataarray[116]) {
    ...
    <tbody class="units">
        <tr>
            ...
            if (($type == 3) || ($type == 17)) $attackerFailed = TRUE;
            else $attackerFailed = FALSE;
            for ($i = 106; $i <= 115; $i++) {
                if ($i == 115 && !$dataarray[116]) {
                    $last = ' last';
                } else {
                    $last = '';
                }
            ...
            if ($dataarray[116]) {
                echo "<td class=\"unit " . ($attackerFailed ? ' none ' : '') . " last\">" . ($attackerFailed ? '?' : $dataarray[116]) . "</td>";
            }
            ?>
        </tr>
    </tbody>
        ...
        <tr>
            ...
            if ($dataarray[116]) {
                if ($dataarray[127] == 0) {
                    ...
                    echo "<td class=\"unit " . ($attackerFailed ? ' none ' : '') . " last\">" . ($attackerFailed ? '?' : $dataarray[127]) . "</td>";
                }
            }
            ?>
        </tr>
