...
if ($_POST and $_GET['action'] == 'change_capital') {
    ...
    $query = mysql_query('SELECT * FROM `users` WHERE `id` = ' . $session->uid);
    ...
    if ($data['password'] == generateHash($pass)) {
        $query1 = mysql_query('SELECT * FROM `vdata` WHERE `owner` = ' . $session->uid . ' AND `capital` = 1');
        $data1 = mysql_fetch_assoc($query1);
        $query2 = mysql_query('SELECT * FROM `fdata` WHERE `vref` = ' . $data1['wref']);
        if ($query2) $data2 = mysql_fetch_assoc($query2);
        if (empty($data2) || $data2['vref'] != $village->wid) {
            for ($i = 1; $i <= 18; ++$i) {
                if ($data2['f' . $i] > 10) {
                    $query2 = mysql_query('UPDATE `fdata` SET `f' . $i . '` = 10 WHERE `vref` = ' . $data2['vref']) or die(mysql_error());
                }
            }

            for ($i = 19; $i <= 40; ++$i) {
                if ($data2['f' . $i . 't'] == 34) {
                    $query3 = mysql_query('UPDATE `fdata` SET `f' . $i . 't` = 0, `f' . $i . '` = 0 WHERE `vref` = ' . $data2['vref']) or die(mysql_error());
                }
            }

            $query3 = mysql_query('UPDATE `vdata` SET `capital` = 0 WHERE `wref` = ' . $data1['wref']);
            $query4 = mysql_query('UPDATE `vdata` SET `capital` = 1 WHERE `wref` = ' . $village->wid);
        }
...
?>
<h1 class="titleInHeader"><?php echo B26; ?> <span class="level"> <?php echo BL_LVL . ' ' . $village->resarray['f' . $id]; ?></span></h1>
<div id="build" class="gid26">
    <div class="build_desc">
        <a href="#" onClick="return Travian.Game.iPopup(26,4, 'gid');" class="build_logo">
            <img class="building big white g26" src="/assets/images/x.gif" alt="<?php echo B26; ?>" title="<?php echo B26; ?>" />
        </a>
        <?php echo B26_DESC; ?>
    </div>
    <?php
    if ($building->getTypeLevel(26) > 0) {
        ...
        if ($village->resarray['f' . $id] >= 10) {
            include("26_train.php");
        } else {
            echo '<div class="c">' . BL_TOFINDNEWVILPAL . '</div>';
        }

        $query = mysql_query('SELECT * FROM `vdata` WHERE `owner` = ' . $session->uid . ' AND `capital` = 1');
        $data = mysql_fetch_assoc($query);
