...
switch ($_GET['action']) {
    ...
    case "success":
        if (isset($_POST['payment_status']) && $_POST['payment_status'] == 'Completed') {
            ...
            $dat = explode("-", $data);
            $from = $dat[1];
            $payment = $database->query("SELECT * FROM `payment` WHERE `uid` = '" . $dat[0] . "' AND `code` = '" . $dat[2] . "'");
            $pay = mysql_fetch_array($payment);
            if (mysql_num_rows($payment) > 0 && $pay['status'] == 0) {
                $database->query("UPDATE `payment` SET `status`='1' WHERE `uid` = '" . $dat[0] . "' AND `code` = '" . $dat[2] . "'");
                $database->query("UPDATE `payment` SET `pemail`='" . $_POST['payer_email'] . "' WHERE `uid` = '" . $dat[0] . "' AND `code` = '" . $dat[2] . "'");
                $to = ADMIN_EMAIL;
                $plyer = mysql_fetch_array($database->query("SELECT * FROM `users` WHERE `id` = '" . $dat[0] . "'"));
			...
			T4DL Team";
                ...
                if ($Player == $session->uid) {
                    $database->query("UPDATE `users` SET gold = gold + $gold WHERE `id` = '" . $Player . "'");
                    $database->query("UPDATE `users` SET boughtgold = boughtgold + $gold WHERE `id` = '" . $Player . "'");
                }
            } elseif ($pay['status'] == 1) {
