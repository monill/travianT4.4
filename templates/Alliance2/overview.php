        ...
        foreach ($memberlist as $member) {
            ...
            if ($aid == $session->alliance) {
                if ((time() - 600) < $member['timestamp']) { // 0 Min - 10 Min
                    echo "<img class='online online1' src=/assets/images/x.gif title='" . AL_ONLINE . "' alt='" . AL_ONLINE . "' />";
                } elseif ((time() - 86400) < $member['timestamp'] && (time() - 600) > $member['timestamp']) { // 10 Min - 1 Days
                    echo "<img class='online online2' src=/assets/images/x.gif title='" . AL_ACTIVE_PLAYERS . "' alt='" . AL_ACTIVE_PLAYERS . "' />";
                } elseif ((time() - 259200) < $member['timestamp'] && (time() - 86400) > $member['timestamp']) { // 1-3 Days
                    echo "<img class='online online3' src=/assets/images/x.gif title='" . AL_ACTIVE_LAST3DAYS . "' alt='" . AL_ACTIVE_LAST3DAYS . "' />";
                } elseif ((time() - 604800) < $member['timestamp'] && (time() - 259200) > $member['timestamp']) { // 3-7 Days
                    echo "<img class='online online4' src=/assets/images/x.gif title='" . AL_ACTIVE_LAST7DAYS . "' alt='" . AL_ACTIVE_LAST7DAYS . "' />";
                } else {
                    echo '<img class="online online5" src="/assets/images/x.gif" title="' . AL_LAZY . '" alt="' . AL_LAZY . '" />';
                }
            }
