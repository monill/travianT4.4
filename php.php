...
abstract class graphBase implements iGraph
{
        ...
        foreach ($properties as $propertyData) {
            ...
            switch ($propertyData['type']) {
                ...
                case 'file':
                    ...
                    break;
                case 'arrayValue':
                    if (!in_array($cfg[$propertyData['name']], $propertyData['values'])) {
                        $cfg[$propertyData['name']] = $propertyData['default'];
                    }
                    break;
                default:
    ...
    public static function getConfigProperties()
    {
        $properties = array(
            ...
            array(
                'name' => 'value-label-visible',
                'description' => 'Set value label visibility',
                'type' => 'boolean',
                'default' => true
            ),
            array(
                'name' => 'value-label-font-size',
            ...
            array(
                'name' => 'box-border-alpha',
                'description' => 'Box border alpha',
                'type' => 'numeric',
                'default' => 0
            ),
            array(
                'name' => 'box-border-visible',
            ...
            array(
                'name' => 'box-background-visible',
                'description' => 'Box background visibility',
                'type' => 'boolean',
                'default' => true
            ),
            array(
                'name' => 'box-background-alpha',
...
abstract class verticalGraphBase extends graphBase
{
    ...
    protected function calculateOffset($dataStructure)
    {
        ...
        if ($this->cfg['value-label-visible']) {
            $result['left'] = ($dataStructure['maxValueLength'] * imagefontwidth($this->cfg['value-label-font-size'])) + 20;
        }

        //Test label
        if ($this->cfg['title-visible']) {
            $result['top'] = ($this->cfg['title'] !== '') ? imagefontheight($this->cfg['title-font-size']) + 20 : $result['top'];
        }

        //Test label
        if ($this->cfg['label-visible']) {
            $result['right'] = ($this->cfg['label'] !== '') ? (strlen($this->cfg['label']) * imagefontwidth($this->cfg['label-font-size'])) + 20 : $result['right'];
        }

        //Test key
        if ($this->cfg['key-visible']) {
            $result['bottom'] = ($dataStructure['maxKeyLength'] * imagefontwidth($this->cfg['key-font-size'])) + 20;
        }
    ...
    protected function calculateGraphBox($datastructure, $points)
    {
        ...
        $result['x2'] = $this->cfg['width'] - $points['offset']['right'];
        $result['y2'] = $this->cfg['height'] - $points['offset']['bottom'];

        //Width
        $result['width'] = (($this->cfg['width'] - $points['offset']['left']) - $points['offset']['right']);

        //Height
        $result['height'] = (($this->cfg['height'] - $points['offset']['top']) - $points['offset']['bottom']);
    ...
    protected function calculateColumnWidth($datastructure, $points)
    {
        ...
        //Calculate col width
        $result['width'] = $points['box']['width'] / $datastructure['cols'];

        //Test col text visible
        if ($result['width'] < imagefontheight($this->cfg['key-font-size'])) {
            $result['show'] = false;
        }
    ...
    protected function calculatePositiveSection($datastructure, $points)
    {
        ...
        if (($datastructure['fakeMin'] < 0) && ($datastructure['fakeMax'] > 0)) {
            ...
            $result['y2'] = (($points['box']['height'] / 100) * $datastructure['positiveSectionPercentage']) + $points['offset']['top'];

            //Quadrant height
            $result['height'] = $result['y2'] - $result['y1'];
        } elseif (($datastructure['fakeMin'] < 0) && ($datastructure['fakeMax'] <= 0)) {
            //Quadrant heights
            $result['height'] = 0;
        } else {
            //Map box to quadrant
    ...
    protected function calculateZero($datastructure, $points)
    {
        ...
        $result['text']['x1'] = $points['box']['x2'] + imagefontwidth($this->cfg['value-label-font-size']);
        $result['text']['y1'] = $result['line']['y2'] - (imagefontheight($this->cfg['value-label-font-size']) / 2);
    ...
    protected function calculateAverage($datastructure, $points)
    {
        ...
        if ($datastructure['avg'] < 0) {
            //Calc
            $avgHeightPerc = ($points['section']['negative']['height'] / 100);
            $avgValPerc = (-$datastructure['avg'] / $datastructure['minDifference']) * 100;
            ...
            //Text
            $result['text']['x1'] = ($points['box']['x1'] - (strlen(round($datastructure['avg'], 2)) * imagefontwidth($this->cfg['value-label-font-size']))) - 2;
            $result['text']['y1'] = $result['line']['y1'] - (imagefontheight($this->cfg['value-label-font-size']) / 2);
        } else {
            //Calc
            $avgHeightPerc = ($points['section']['positive']['height'] / 100);
            $avgValPerc = ($datastructure['plusDifference'] > 0) ? ($datastructure['avg'] / $datastructure['plusDifference']) * 100 : 0;
    ...
    protected function calculateTitle($datastructure, $points)
    {
        ...
        $result['x1'] = $points['box']['x1'];
        $result['y1'] = ($points['offset']['top'] / 2) - (imagefontheight($this->cfg['title-font-size']) / 2);
    ...
    protected function calculateLabel($datastructure, $points)
    {
        ...
        $result['y1'] = ($points['box']['y1'] - imagefontheight($this->cfg['label-font-size'])) - 2;
    ...
    protected function calculateValueLabels($datastructure, $points)
    {
        ...
        /*
			...
			2);
		*/
        $result['min']['y1'] = $points['box']['y2'] - (imagefontheight($this->cfg['value-label-font-size']) / 2);
    ...
    protected function drawHorizontaldividers($graphPoints)
    {
        ...
        if ($this->cfg['horizontal-divider-visible']) {
            //Set offset
            $offset = ($graphPoints['box']['height'] / 8);
            //Allocate colors
    ...
    protected function drawAverage($graphPoints, $dataStructure)
    {
        ...
        if ($this->cfg['average-line-visible']) {
            //allocateColor
            $avgLineColor = $this->allocateColor($this->cfg['average-line-color'], $this->cfg['average-line-alpha']);
    ...
    protected function drawZero($graphPoints)
    {
        ...
        if ($this->cfg['zero-line-visible']) {
            ...
            //allocateColor
            $color = $this->allocateColor($this->cfg['zero-line-color'], $this->cfg['zero-line-alpha']);
...
abstract class horizontalGraphBase extends graphBase
{
    ...
    protected function calculateOffset($dataStructure)
    {
        ...
        //Test value
        if ($this->cfg['value-label-visible']) {
            $result['bottom'] = ($dataStructure['maxValueLength'] * imagefontwidth($this->cfg['value-label-font-size'])) + 20;
        }
        ...
        //Test key
        if ($this->cfg['key-visible']) {
            $result['left'] = ($dataStructure['maxKeyLength'] * imagefontwidth($this->cfg['key-font-size'])) + 20;
        }
    ...
    protected function calculateColumnHeight($datastructure, $points)
    {
        ...
        //Test col text visible
        if ($result['height'] < imagefontheight($this->cfg['key-font-size'])) {
            $result['show'] = false;
        }
    ...
    protected function calculateAverage($datastructure, $points)
    {
        ...
        if ($datastructure['avg'] < 0) {
            //Calc
            $avgHeightPerc = ($points['section']['negative']['width'] / 100);
            $avgValPerc = (-$datastructure['avg'] / $datastructure['minDifference']) * 100;
            $avgValueHeight = $avgHeightPerc * $avgValPerc;
    ...
    protected function drawZero($graphPoints)
    {
        ...
        if ($this->cfg['zero-line-visible']) {
            ...
            if ($graphPoints['zero']['line']['x1'] != $graphPoints['box']['x1']) {
    ...
    {
        $posColor = $this->allocateColor($this->hex2Rgb('00FF00'), $this->cfg['zero-line-alpha']);

        $negColor = $this->allocateColor($this->hex2Rgb('FF0000'), $this->cfg['zero-line-alpha']);
...
class verticalLineGraph extends verticalGraphBase
{
    ...
    protected function drawColumn($colPoint, $columnColor, $columnShadowColor)
    {
        ...
        //taviz khat be rang//
        // imagefilledpolygon($this->ip, $colPoint['line']['x1'], $colPoint['line']['y1'], $colPoint['line']['x2'], $colPoint['line']['y2'], $columnColor);
