<h1 class="titleInHeader"><?php echo B16; ?> <span class="level"> <?php echo BL_LVL . ' ' . $village->resarray['f' . $id]; ?></span></h1>
<div id="build" class="gid16">
    ...
    if (isset($_GET['disable'])) {
        $database->query("UPDATE `vdata` SET `evasion`=0 WHERE `wref` = '" . $village->wid . "'");
        header("Location: build.php?id=39&t=100");
        echo "<SCRIPT language='JavaScript'>window.location='build.php?id=39&t=100';</SCRIPT>";
    }
    if (isset($_GET['enable'])) {
        $database->query("UPDATE `vdata` SET `evasion`=1 WHERE `wref` = '" . $village->wid . "'");
        header("Location: build.php?id=39&t=100");
        echo "<SCRIPT language='JavaScript'>window.location='build.php?id=39&t=100';</SCRIPT>";
    }
    $evasion = mysql_fetch_array($database->query("SELECT * FROM `vdata` WHERE `wref` = '" . $village->wid . "'"));
    ?>
    ...
    <div id="raidList">
        <div class="options">
            ...
            <br><br><br>
            <input type="checkbox" class="check" name="hideShow" onclick="window.location.href = '?id=39&t=100&<?php if ($evasion['evasion'] == 1) {
                                                                                                                    echo "disable";
                                                                                                                } else {
                                                                                                                    echo "enable";
                                                                                                                } ?>';" <?php if ($evasion['evasion'] == 1) {
