...
foreach ($vol_location as $loc) {
    ...
    $q = "UPDATE wdata set `fieldtype` = $fieldtyp , `image` = '$image' , `pos` ='$pos' WHERE x = '$x' AND y = '$y'";
...
for ($i = 0; $i <= 198; $i += $rand_i) {
    ...
    for ($z = 0; $z <= 198; $z += $rand_z) {
        ...
        if (isset($lists)) {
            $imgloc = "/assets/images/mkarte/" . $lists . ".gif";
            ...
            if ($start_x < 11820 && $start_y < 11820) {
                ...
                if ($notallow == 0) {
                    foreach ($temp as $temporary) {
                        ...
                        $q = "UPDATE wdata set $advance `image` = '$notil' , `pos` ='$t' WHERE x = '$x' AND y = '$y'";
                        mysql_query($q) or die(mysql_error());
                        $tiles++;
                    }
                }
