...
$noticeClass = array(
    NOTICE1, NOTICE2, NOTICE3, NOTICE4, NOTICE5, NOTICE6, NOTICE7, NOTICE8, NOTICE9, NOTICE10, NOTICE11, NOTICE12, NOTICE13, NOTICE14, NOTICE15,
...
if ($page == "1" && $lastPage == "1") {
    ...
    $paginationDisplay .= '<img alt="' . PREVPAGE . '" src="/assets/images/x.gif" class="previous disabled">';
    $paginationDisplay .= $centerPages;
    $paginationDisplay .= '<a class="next" href="' . $_SERVER['PHP_SELF'] . '?page=2"><img alt="' . NEXTPAGE . '" src="/assets/images/x.gif"></a> ';
    $paginationDisplay .= '<a class="last" href="' . $_SERVER['PHP_SELF'] . '?page=3"><img alt="' . LASTPAGE . '" src="/assets/images/x.gif"></a>';
} elseif ($page != "1" && $page != $lastPage) {
    $paginationDisplay .= '<a class="first" href="' . $_SERVER['PHP_SELF'] . '?page=1"><img alt="' . FIRSTPAGE . '" src="/assets/images/x.gif"></a> ';
    $paginationDisplay .= '<a class="previous" href="' . $_SERVER['PHP_SELF'] . '?page=' . $previous . '"><img alt="' . PREVPAGE . '" src="/assets/images/x.gif"></a>';
    $paginationDisplay .= $centerPages;
    $paginationDisplay .= '<a class="next" href="' . $_SERVER['PHP_SELF'] . '?page=' . $nextPage . '"><img alt="' . NEXTPAGE . '" src="/assets/images/x.gif"></a> ';
    $paginationDisplay .= '<a class="last" href="' . $_SERVER['PHP_SELF'] . '?page=' . $lastPage . '"><img alt="' . LASTPAGE . '" src="/assets/images/x.gif"></a>';
...
if ($query == 0) {
    ...
    while ($row = mysql_fetch_array($sql2)) {
        ...
        if ($viewed == 0) {
            $outputList .= '<img src="/assets/images/x.gif" class="messageStatus messageStatusUnread" title="unread">';
        } else {
            $outputList .= '<img src="/assets/images/x.gif" class="messageStatus messageStatusRead" title="read">';
        }

        if ($type == 9) {
            $outputList .= "<img src=\"/assets/images/x.gif\" class=\"iReport iReport21\" alt=\"" . $noticeClass[$ntype] . "\" title=\"" . $noticeClass[$ntype] . "\" />";
        } elseif ($type == 10) {
            $outputList .= "<img src=\"/assets/images/x.gif\" class=\"iReport iReport11\" alt=\"" . $noticeClass[$ntype] . "\" title=\"" . $noticeClass[$ntype] . "\"  >";
        } else {
            $outputList .= "<img src=\"/assets/images/x.gif\" class=\"iReport iReport$type\" alt=\"" . $noticeClass[$type] . "\" title=\"" . $noticeClass[$type] . "\" />";
        }
        ...
        if ($type == 1 || $type == 2 || $type == 5 || $type == 6 || $type == 7) {
            ...
            $outputList .= "<a class=\"reportInfoIcon\" href='berichte.php?id=" . ($id) . "'><img title=\"" . $carry . "\" src=\"/assets/images/x.gif\" class=\"reportInfo carry " . $style . "\"></a>";
        } elseif ($type == 9) {
            $btype = $dataarray[1];
            $type = $dataarray[2];
            ...
            if ($dataarray[1] != 'dead') {
                if ($dataarray[1] != '') {
                    $outputList .= "<div class=\"reportInfoIcon\"><img title=\"" . $name . " (" . $dataarray[3] . "x)\" src=\"/assets/images/x.gif\" class=\"reportInfo itemCategory itemCategory_" . $typeArray[$dataarray[1]] . "\"></div>";
                }
            } else {
                $outputList .= "<div class=\"reportInfoIcon\"><img src=\"/assets/images/x.gif\" class=\"reportInfo adventureDifficulty0\" title=\"" . HS_HERODEAD . "\"></div>";
            }
        }
...
<form method="post" action="berichte.php" name="msg">
    ...
    <div class="footer">
        <?php if ($session->plus) { ?>
            <div id="markAll">
                <input class="check" type="checkbox" id="sAll" onclick="$(this).up('form').getElements('input[type=checkbox]').each(function(element){element.checked = this.checked;}, this);">
...
<div id="auction">
    <div class="auctionAdventureBarText">
        Size of the report box<br>
        <div class="dis">Reports will be automatically deleted when full capacity.</div>
    </div>
