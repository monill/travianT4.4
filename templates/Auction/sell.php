<div id="auction">
        ...
        <div id="filter">
            <div class="boxes boxesColor gray">
                ...
                <div class="boxes-contents cf">
                    <div class="wrapper">
                        <div class="silver">
                            <img title="<?php echo HDR_SILVER; ?>" class="silver" src="/assets/images/x.gif"> <?php $now = $database->getAuctionSilver($session->uid);
                                                                                                                echo ($session->silver - $now['silver']); ?> / <?php echo $session->silver; ?>
                        </div>
    ...
    if ($query == 0) {
        ...
        while ($row = mysql_fetch_array($sql)) {
            ...
            $silver = $row["silver"];
            $time = $row["time"];
            $usedtime = AUCTIONTIME - ($time - time());

            include "templates/Auction/alt.php";

            if (($usedtime > (AUCTIONTIME / 10)) || ($bids != 0)) {
                $outputList .= "<tr><td class=\"delete\"><img class=\"del inactive\" src=\"/assets/images/x.gif\" title=\"" . HR_CANCEL . "\"></td><td class=\"icon\"><img class=\"itemCategory itemCategory_" . $typeArray[$btype] . "\" src=\"/assets/images/x.gif\" title=\"" . $title . "\"></td>";
            } else {
                $outputList .= "<tr><td class=\"delete\"><a href=\"?action=sell&abort=" . $id . "\"><img class=\"del\" src=\"/assets/images/x.gif\" title=\"" . HE_CANCEL . "\"></a></td><td class=\"icon\"><img class=\"itemCategory itemCategory_" . $typeArray[$btype] . "\" src=\"/assets/images/x.gif\" title=\"" . $title . "\"></td>";
            }
            ...
            $outputList .= "</td>";
            $outputList .= "<td class=\"silver\" title=\"" . round($silver / $num, 2) . " " . HR_FOREACHUNIT . "\">" . $silver . "</td>";
    ...
    $outputList = '';
    if ($query2 == 0) {
        $outputList .= "<span class='none'>" . HR_FINISHEDAUCTIONS . "</span>";
    } else {
    ...
    if ($page == "1" && $lastPage == "1") {
        $paginationDisplay .= '<img alt="' . NEXTPAGE . '" src="/assets/images/x.gif" class="first disabled"> ';
        ...
        $paginationDisplay .= '<img alt="' . PREVPAGE . '" src="/assets/images/x.gif" class="previous disabled">';
        $paginationDisplay .= $centerPages;
        $paginationDisplay .= '<a class="next" href="?action=sell&page=' . $nextPage . '"><img alt="' . NEXTPAGE . '" src="/assets/images/x.gif"></a> ';
        $paginationDisplay .= '<a class="last" href="?action=sell&page=' . $lastPage . '"><img alt="' . LASTPAGE . '" src="/assets/images/x.gif"></a>';
    } elseif ($page != "1" && $page != $lastPage) {
        $paginationDisplay .= '<a class="first" href="?action=sell&page=1"><img alt="' . FIRSTPAGE . '" src="/assets/images/x.gif"></a> ';
        $paginationDisplay .= '<a class="previous" href="?action=sell&page=' . $previous . '"><img alt="' . PREVPAGE . '" src="/assets/images/x.gif"></a>';
        $paginationDisplay .= $centerPages;
        $paginationDisplay .= '<a class="next" href="?action=sell&page=' . $nextPage . '"><img alt="' . NEXTPAGE . '" src="/assets/images/x.gif"></a> ';
        $paginationDisplay .= '<a class="last" href="?action=sell&page=' . $lastPage . '"><img alt="' . LASTPAGE . '" src="/assets/images/x.gif"></a>';
    ...
    $timer = 1;
    if ($query == 0) {
        $outputList .= "<td colspan=\"6\" class=\"none\"><center>" . HR_NOSALESFOUND . "</center></td>";
    } else {
        while ($row = mysql_fetch_array($sql2)) {
            $id = $row["id"];
            $owner = $row["owner"];
            ...
            $outputList .= "<tr><td class=\"icon\"><img class=\"itemCategory itemCategory_" . $typeArray[$btype] . "\" src=\"/assets/images/x.gif\" title=\"" . $name . "||" . $title . "\"></td>";
    ...
    ?>
    <h4 class="auctionEnded spacer"><?php echo HR_FINISHEDAUCTIONS; ?></h4>
    ...
    <script type="text/javascript">
        Travian.Game.HeroAuction = new(new Class({
            alreadyOpen: false,
            textSingle: '<?php echo AL_CONFIRM; ?>',
            ...
            noMoreAuctions: function() {
                ('<?php echo sprintf(HR_MAXSELLDESC, MAXSELL); ?> ').dialog({
                    ...
                    buttonTextOk: '<?php echo AL_CONFIRM; ?>'
                });
            },
            sellItem: function(id, amount) {
                ...
                html.dialog({
                    relativeTo: $('content'),
                    elementFoucs: 'sellAmount',
                    buttonTextOk: '<?php echo HR_YES; ?>',
                    buttonTextCancel: '<?php echo HR_NO; ?>',
                    title: '<?php echo AL_CONFIRM; ?>:',
