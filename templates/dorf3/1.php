    ...
    <tbody>
        ...
        foreach ($varray as $vil) {
            ...
            if (count($incoming_attacks) > 0) {
                ...
                if ($inc_atts > 0) {
                    $att = '<a href="build.php?newdid=' . $vid . '&id=39"><img class="att1" src="/assets/images/x.gif" title="' . count($incoming_attacks) . 'x ' . AT_ATTACKS . (count($incoming_attacks) > 1 ? '' : '') . '" alt="' . count($incoming_attacks) . 'x ' . AT_ATTACKS . (count($incoming_attacks) > 1 ? '' : '') . '"></a>';
                }
            }
            foreach ($jobs as $b) {
                $bui = '<a href="build.php?newdid=' . $vid . '&id=' . $b['field'] . '"><img class="bau" src="/assets/images/x.gif" title="' . $building->procResType($b['type']) . '" alt="' . $building->procResType($b['type']) . '"></a>';
            }

            foreach ($unit as $c) {
                $tro = '<a href="build.php?newdid=' . $vid . '&gid=19"><img class="unit u' . $c['unit'] . '" src="/assets/images/x.gif" title="' . $c['amt'] . 'x ' . $technology->getUnitName($c['unit']) . '" alt="' . $c['amt'] . 'x ' . $technology->getUnitName($c['unit']) . '"></a>';
            }
