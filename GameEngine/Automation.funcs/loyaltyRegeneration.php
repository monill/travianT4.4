...
function loyaltyRegeneration()
{
    ...
    $array = array();
    $q = "SELECT `wref`,`loyalty`,`lastupdate` FROM vdata WHERE loyalty < 100 LIMIT 50";
    ...
    if (!empty($array)) {
        foreach ($array as $loyalty) {
            ...
            $q = "UPDATE vdata SET loyalty = $newloyalty WHERE wref = '" . $loyalty['wref'] . "'";
            $database->query($q);
        }
    }
    $array = array();
    $q = "SELECT `loyalty`,`conqured`,`lastupdated`,`wref` FROM odata WHERE loyalty<100 LIMIT 50";
    $array = $database->query_return($q);
    if (!empty($array)) {
        foreach ($array as $loyalty) {
            ...
            $q = "UPDATE odata SET loyalty = $newloyalty WHERE wref = '" . $loyalty['wref'] . "'";
            $database->query($q);
        }
    }
    $q = "UPDATE vdata SET loyalty = 125 WHERE loyalty>125";
    $database->query($q);
}
