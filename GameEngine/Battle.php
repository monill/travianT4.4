...
class Battle
{
    ...
    public function procSim($post)
    {
        ...
        if (isset($post['attackertribe']) && isset($post['defendertribe'])) {
            ...
            for ($i = 1; $i <= 5; $i++) {
                if (isset($post['reinf_' . $i])) {
                    array_push($enforcestribes, $i);
                }
            }
            $participantstribes = array();
            for ($i = 1; $i <= 5; $i++) {
                if (isset($post['parti_' . $i])) {
                    array_push($participantstribes, $i);
                }
            }
    ...
    private function simulate($post)
    {
        ...
        switch ($Defender['tribe']) {
            case 1:
                $Defender['buildings']['f40t'] = 31;
                break;
            case 2:
            case 5:
                $Defender['buildings']['f40t'] = 32;
                break;
            case 3:
                $Defender['buildings']['f40t'] = 33;
                break;
        }
    ...
    function calculateBattle($Attacker, $Defender, $Enforces, $Participants = null)
    {
        ...
        if ($Attacker['attackdata']['attack_type'] != 1 && $Defender['tribe'] == 3 && isset($Defender['u199']) && $Defender['u199'] > 0) {
            ...
            $trapped = array();
            for ($i = $start; $i <= $end; $i++) {
                $marr['u' . $i] = $Attacker['u' . $i];
                $trapped['u' . $i] = 0;
            }
            do {
                $max = max($marr);
                $key = array_search($max, $marr);
                if ($marr[$key] > 0) {
                ...
                for ($ec = 0; $ec < $enforcesCount; $ec++) {
                    ...
                    for ($i = $start; $i <= $end; $i++) {
                        if ($enforce['u' . $i] > 0) {
                            ...
                            if (!isset($result['had']['enforces'][$ec]['u' . $i])) {
                                $result['had']['enforces'][$ec]['u' . $i] = 0;
                            }
                            ...
                            if (!isset($result['had']['enforces'][$ec]['count'])) {
                                $result['had']['enforces'][$ec]['count'] = 0;
                            }
                            $result['had']['enforces'][$ec]['count'] += $enforce['u' . $i];
                            if (!isset($result['had']['enforces'][$ec]['pop'])) {
                                $result['had']['enforces'][$ec]['pop'] = 0;
                            }
        ...
        if (isset($Defender['villagearties']) && !empty($Defender['villagearties'])) {
            foreach ($Defender['villagearties'] as $arte) {
                if ($arte['effecttype'] == 5) {
                    $confArteEff = $arte['effect'];
                    $hasConfArte = true;
                }
            }
        }
        ...
        if ($Attacker['attackdata']['attack_type'] == 1) {
            ...
            if (isset($Attacker['villagearties']) && !empty($Attacker['villagearties'])) {
                foreach ($Attacker['villagearties'] as $arte) {
                    if ($arte['effecttype'] == 5) {
                        $attSpyArteEff = $arte['effect'];
                    }
                }
            }
        ...
        if ($Attacker['attackdata']['attack_type'] == 3) {
            ...
            $targetBuilding = array(
                0 => array('f' => 0, 'ft' => 0, 'lvl' => 0),
                ...
                if ($Attacker['attackdata']['ctar2'] > 0 && $Attacker['attackdata']['ctar2'] != 255) {
                    ...
                    for ($i = 0; $i <= 39; $i++) {
                        if ($buildarray['f' . $indexArray[$i] . 't'] == $Attacker['attackdata']['ctar2'] && $buildarray['f' . $indexArray[$i]] != 0) {
                            ...
                            $targetBuilding[1]['ft'] = $buildarray['f' . $indexArray[$i] . 't'];
                            $targetBuilding[1]['lvl'] = $buildarray['f' . $indexArray[$i]];
                            break;
                        }
                    }
            ...
            if ($catpCount > 0 && ($targetBuilding[0]['f'] != 0 || $targetBuilding[1]['f'] != 0 || $targetBuilding[0]['ft'] != 0 || $targetBuilding[1]['ft'] != 0)) {
                ...
                $BuildLevelStrength = array(
                    0 => 0, 1, 2, 2, 3, 4, 6, 8, 10, 12, 14, 17, 20, 23, 27, 31, 35, 39, 43, 48, 53, 58, 64, 70, 76, 88, 95, 102, 109, 117, 125, 133, 141, 149, 158, 167, 176, 186, 196, 206, 216, 226, 237, 248, 259, 271, 283, 295, 307, 319, 332, 345, 358, 372, 386,
                ...
                if ($catpCount1 > 0 && ($targetBuilding[0]['f'] != 0 || $targetBuilding[0]['ft'] != 0) && $Attacker['attackdata']['ctar1'] != 255) {
                    if ($targetBuilding[0]['ft'] == 40) {
                        $tmpAAP = 1;
                    } else {
                        $tmpAAP = $archArtePower;
                    }
                    ...
                    $BuildingLevelMax = 20;
                    if (($Defender['iscapital'] != 1 && $Attacker['attackdata']['ctar1'] <= 18) || $Attacker['attackdata']['ctar1'] == 36) {
                        $BuildingLevelMax = 10;
                    }
                    if ($Attacker['attackdata']['ctar1'] >= 5 && $Attacker['attackdata']['ctar1'] <= 9) {
                        $BuildingLevelMax = 5;
                    }
                    if ($Attacker['attackdata']['ctar1'] == 40) {
                        $BuildingLevelMax = 100;
                    }
        ...
        if (isset($Enforces) && count($Enforces) > 0) {
            ...
            for ($ec = 0; $ec < $enforcesCount; $ec++) {
                ...
                for ($i = 1; $i <= 50; $i++) {
                    if (!isset($result['casualties']['enforces'][$ec]['u' . $i])) {
                        $result['casualties']['enforces'][$ec]['u' . $i] = 0;
                    }
                    $result['casualties']['enforces'][$ec]['u' . $i] += round($result[2] * (isset($result['had']['enforces'][$ec]['u' . $i]) ? $result['had']['enforces'][$ec]['u' . $i] : 0));
                    if (!isset($result['reminders']['enforces'][$ec]['u' . $i])) {
                        $result['reminders']['enforces'][$ec]['u' . $i] = 0;
                    }
                    ...
                    if (!isset($result['casualties']['enforces'][$ec]['count'])) {
                        $result['casualties']['enforces'][$ec]['count'] = 0;
                    }
                    $result['casualties']['enforces'][$ec]['count'] += round($result[2] * (isset($result['had']['enforces'][$ec]['count']) ? $result['had']['enforces'][$ec]['count'] : 2));
                    if (!isset($result['reminders']['enforces'][$ec]['count'])) {
                        $result['reminders']['enforces'][$ec]['count'] = 0;
                    }
                    ...
                    if (!isset($result['casualties']['enforces'][$ec]['pop'])) {
                        $result['casualties']['enforces'][$ec]['pop'] = 0;
                    }
                    $result['casualties']['enforces'][$ec]['pop'] += round($result[2] * (isset($result['had']['enforces'][$ec]['pop']) ? $result['had']['enforces'][$ec]['pop'] : 0));
                    if (!isset($result['reminders']['enforces'][$ec]['pop'])) {
                        $result['reminders']['enforces'][$ec]['pop'] = 0;
                    }
        ...
        if ($Attacker['attackdata']['attack_type'] == 4) {
            if ($Defender['isoasis'] && $result['reminders']['defender']['overall']['count'] <= 0) {
                ...
                $difx = $toOases['x'] - $fromVillage['x'];
                $dify = $toOases['y'] - $fromVillage['y'];
                ...
                if ($distance < 4.9497474683058326708059105347339) {
                    if (isset($result['reminders']['attacker']['hero'])) {
                        if ($Attacker['avoasisslots'] <= 0) {
                            ...
                            if ($Defender['uid'] == 3) {
                                $result['hero']['loyaltychange'] = 100;
                            } else {
                                $result['hero']['loyaltychange'] = floor(100 / min(3, (4 - $database->VillageOasisCount($Attacker['attackdata']['to']))));
                            }
        ...
        if ($result['had']['defender']['res_array']) {
            ...
            foreach ($had_array as $key => $value) {
                $av_array[$key] = max($value - $cranny_eff, 0);
            }
