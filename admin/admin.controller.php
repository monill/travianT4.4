...
class adminController
{
    ...
    function act($req)
    {
        ...
        switch ($req['action']) {
            case 'savegold':
                $req['gold'] = intval($req['gold']);
                $req['boughtgold'] = intval($req['boughtgold']);
                $req['giftgold'] = intval($req['giftgold']);
                $req['transferedgold'] = intval($req['transferedgold']);
                $req['seggold'] = intval($req['seggold']);
                $req['usedgold'] = intval($req['usedgold']);
                $this->model->saveGold($req);
                header("Location: ?p=player&uid=" . $req['uid']);
                exit;
                break;
            case 'savesilver':
                $req['silver'] = intval($req['silver']);
                $req['giftsilver'] = intval($req['giftsilver']);
                $req['gessilver'] = intval($req['gessilver']);
                $req['bisilver'] = intval($req['bisilver']);
                $req['sisilver'] = intval($req['sisilver']);
            ...
            case 'savetroops':
                ...
                header("Location: ?p=village&did=" . $req['id']);
                exit;
                break;
            case 'delGoldClub':
            ...
            case 'addGoldClub':
                ...
                break;
            case 'recountPop':
                $this->model->recountPop($req['did']);
                break;
            case 'recountPopUsr':
                $this->model->recountPopUser($req['uid']);
                break;
            case 'stopDel':
                break;
            case 'delVil':
                $this->model->delVillage($req['did']);
                break;
            case 'delBan':
                $this->model->delBan($req['uid']);
                break;
            case 'addBan':
                if ($req['time']) {
                    $end = time() + $req['time'];
            ...
            case 'delPlayer':
                ...
                break;
            case 'punish':
                $this->model->punish($req);
                header("Location: " . $_SERVER['HTTP_REFERER']);
                exit;
                break;
            case 'addVillage':
                $this->model->addVillage($req);
                header("Location: " . $_SERVER['HTTP_REFERER']);
                exit;
                break;
        }
