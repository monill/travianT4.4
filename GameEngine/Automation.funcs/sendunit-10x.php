...
function operateBattleResult(&$battleresult)
{
    ...
    $res = sendBattleReports($battleresult);

    // Effect Battle LossGain
    effectBattleLossGain($battleresult);

    //fix hero damage by shadow
    //var_dump($res);die;
    if ($res != null) {
        $result = explode("/", $res);
        if ($result[0] == "damagehero" && $result[1] > 0) {
            $value = $result[1];
            $uid = $battleresult['had']['attacker']['uid'];
            $database->modifyHero2('health', $value, $uid, 2);
        }
    }
}
    ...
    if ($data['attack_type'] == 3) {
        ...
        if (isset($battleresult['casualties']['defender']['ctar'])) {
            ...
            for ($i = 0; $i < $ctarCount; $i++) {
                ...
                $ref = $data['to'];
                $field = 'f' . $ctar['f'];
                $value = $battleresult['reminders']['defender']['buildings']['f' . $ctar['f']];
...
function sendBattleReports(&$battleresult)
{
    ...
    if (isset($battleresult['hero']['msg']) && $battleresult['hero']['msg2'] == 1) {
        //if($battleresult['hero']['msg2'] == 1){
        //$database->modifyHero2('health', $battleresult['hero']['damage'], $battleresult['had']['attacker']['uid'], 2);
        //$database->modifyHero2('health', 0, $battleresult['had']['attacker']['uid'], 2);
        //$database->modifyHero2('dead', 0, $battleresult['had']['attacker']['uid'], 0);
        return "damagehero/" . $battleresult['hero']['damage'];
        // }
    }
}
